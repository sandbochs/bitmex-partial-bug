...
module.exports = function createSocket(options, bmexClient) {
  ...
  wsClient.onmessage = function(data) {
    ...
    if (tableNoSymbol) {
      ...
      console.log('table no symbol', data.table)
      ...
      console.log('table symbol', data.table)
...
function emitSplitData(emitter, data) {
  ...
  console.log('emit split', table, action, data)
  ...
  console.log('esd data.data', data.data)
  console.log('eSD symoblData', symbolData)
